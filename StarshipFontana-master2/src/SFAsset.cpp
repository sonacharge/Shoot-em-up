...
SFAsset::SFAsset(SFASSETTYPE type, std::shared_ptr<SFWindow> window): type(type), sf_window(window) {
  ...
  case SFASSET_COIN:
    ...
    break;
  case SFASSET_WALL:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/wall.png");	//adding wall sprite
    break;
  case SFASSET_BACKGROUND:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/background.png");	//adding wall sprite
    break;
  }
...
//Moving the player in all four directions

void SFAsset::GoNorthPlayer() {
  int w, h;
...
void SFAsset::AlienMovementLeft() {
	 int w, h;
	 SDL_GetRendererOutputSize(sf_window->getRenderer(), &w, &h);
...
void SFAsset::AlienMovementRight() {
	  int w, h;
	  SDL_GetRendererOutputSize(sf_window->getRenderer(), &w, &h);
